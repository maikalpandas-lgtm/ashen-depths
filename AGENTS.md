...
📍 **Начинать с [`docs/NEXT.md`](docs/NEXT.md)** — что осталось сделать и как
это проверять. Здесь ниже — только ловушки и соглашения.
...
⚠️ **Фон должен быть ЧИСТОЙ `#FF00FF`.** Портреты батча 6 приехали на горячем
розовом `(249,17,165)`, и заливка утекла через тёплый контурный свет прямо в
светлую кожу — щека, подбородок и блик на носу просто исчезли. Допуск не
помогал: утечка топологическая. Теперь резак заделывает дыры сам
(`seal_holes`), но проверять цвет фона всё равно надо — `detect_key()` печатает
...
диске. В редакторе всё выглядит нормально, в экспорте спрайт пропадает. Порядок:
`ResourceLoader.exists()` → `load()`, и только потом `Image.load()` как запасной
путь для неимпортированного ассета.
...
- ✅ **Запускать игру для проверки МОЖНО** (запрет снят 25.07.2026). Раньше было
  «юзер запускает сам»; теперь агент поднимает игру и проверяет визуал через
  `tools/game.py` — иначе кадры зависят от того, играет ли юзер в этот момент.
  Закрывать за собой процесс, когда проверка закончена.
- 🔌 **Живой канал в запущенную игру: `tools/game.py`.** Юзер держит окно игры открытым, агент сам снимает кадры и кликает — не «чинит вслепую» и не гоняет юзера за скринами.
  ```bash
  python3 tools/game.py shot --name combat   # кадр -> shots/auto/combat.png, читать Read'ом
  python3 tools/game.py eval 'get_viewport().size'
  python3 tools/game.py click 640 400
  python3 tools/game.py drag 400 660 560 400 # сыграть карту в моба
  ```
  Под капотом — автолоад `scripts/mcp/mcp_interaction_server.gd` (TCP 127.0.0.1:9090).
  В headless он не поднимается, так что тесты можно гонять при открытой игре.
  `eval` оборачивается в тело функции — нужен `return`, клиент подставляет сам.
  🚀 **Прямо в бой, минуя меню:** `tools/fight.sh` (флаги `--forest`,
  `--floor 4`, `--hero volhv`). Проверять боевой визуал через титры + выбор
  героя + поиск стаи — так дорого, что проверка просто не делается.
  ⚠️ `eval` меняет ЖИВУЮ игру, в которую играет юзер. Замерять — да, но за собой
  убирать: создал пробную камеру — верни `current` боевой/игроковой обратно.

⚠️ **`name` присваивать ПОСЛЕ `add_child`**, иначе Godot переименует дубликаты в
`@Node3D@N`. На живой карте было 406 факелов и ровно ОДИН с именем `WallTorch`.
Те же грабли уже ловили на холдерах врагов. Искать пачками — через группы.
